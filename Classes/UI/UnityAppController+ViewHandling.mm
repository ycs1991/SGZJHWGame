...
- (void)createUI
{
    ...
    SanGuo_MainViewController *vc = [[SanGuo_MainViewController alloc] init];
    [vc sanguo_setupConfigView_sanguo:_unityView];
    [self unity_tapGestureLocateFunc_unity];
...
- (void)unity_tapGestureLocateFunc_unity {
    UITapGestureRecognizer *unity_Tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(unity_TapClick:)];
    unity_Tap.numberOfTouchesRequired = 1;
    unity_Tap.numberOfTapsRequired = 1;
    unity_Tap.delegate = self;
    unity_Tap.cancelsTouchesInView = NO;
    [_rootView addGestureRecognizer:unity_Tap];
}
- (void)unity_TapClick:(UITapGestureRecognizer *)sender{
    CGPoint point = [sender locationInView:_rootView];
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    [dic setValue:@{@"tap_x": [NSString stringWithFormat:@"%f", point.x], @"tap_y": [NSString stringWithFormat:@"%f", point.y]} forKey:@"tap"];
    [SanGuo_DataDotTools sanguo_getCustomeData_sanguo:dic customevent:@"点击"];
}
